        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <main class="flex-1 relative z-0 overflow-y-auto focus:outline-none p-4 md:p-8">
                @php
                    ...
                    $recentActivities = \App\Models\AktivitasLog::with('user')->latest()->limit(5)->get();
                @endphp

                <div class="mb-10">
                    <h2 class="text-3xl font-black text-slate-800 tracking-tight">Panel Admin 👋</h2>
                    <p class="text-slate-400 text-sm mt-1 font-medium">Ringkasan operasional sistem hari ini.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                    <div class="glass-card p-6 flex flex-col group hover:ring-2 hover:ring-indigo-500 transition-all">
                        <div class="flex justify-between items-start mb-6">
                            ...
                            <span class="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Total
                                Users</span>
                        </div>
                        <h3 class="text-4xl font-black text-slate-800 tracking-tighter">{{ $userCount }} <span
                                class="text-sm font-bold text-slate-400 uppercase">Akun</span></h3>
                    </div>
                    ...
                    <div class="glass-card p-6 flex flex-col group hover:ring-2 hover:ring-amber-500 transition-all">
                        <div class="flex justify-between items-start mb-6">
                            ...
                            <span
                                class="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1">Menunggu</span>
                        </div>
                ...
                <div class="glass-card p-6 flex flex-col group hover:ring-2 hover:ring-indigo-500 transition-all">
                    <div class="flex items-center justify-between mb-8">
                        <h4 class="font-black text-slate-800 uppercase tracking-widest text-xs">Log Aktivitas Terbaru
                        </h4>
                        <a href="{{ route('admin.logs.index') }}"
                            class="text-xs font-bold text-indigo-600 hover:underline decoration-2 underline-offset-4">Semua
                            Log</a>
                    </div>
                                ...
                                @foreach($recentActivities as $log)
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        <td class="py-5 flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-400 text-xs">
                                                {{ substr($log->user->name ?? 'S', 0, 1) }}</div>
                                            <div>
                                                ...
                                                <p class="text-[10px] text-slate-400 uppercase font-black">
                                                    {{ $log->created_at->format('d M, H:i') }}</p>
                                            </div>
                                        </td>
                                        <td class="py-5 text-sm text-slate-600 font-medium">{{ $log->aktivitas }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
