...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#fcfdfe] antialiased">
        <div class="min-h-screen flex">
            ...
            <!-- Main Content -->
            <div class="flex-1 flex flex-col min-w-0 overflow-hidden" x-data="{ addModal: false, editModal: false, editData: {id: null, name: ''} }">
                <!-- Mobile Top Nav -->
                ...
                <main class="flex-1 relative z-0 overflow-y-auto focus:outline-none p-4 md:p-8">
                    <div class="mb-10 flex items-end justify-between">
                        <div>
                            <h2 class="text-3xl font-black text-slate-800 tracking-tight">Manajemen Kategori</h2>
                            <p class="text-slate-400 text-sm mt-1 font-medium">Kelola kategori alat yang tersedia di sistem.</p>
                        </div>
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="bg-slate-50/50 border-b border-slate-100">
                                    <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Nama Kategori</th>
                                    <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Jenis Alat</th>
                                    <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Total Stok</th>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @foreach($kategoris as $kategori)
                                    <tr class="group hover:bg-slate-50/30 transition-colors">
                                        <td class="px-8 py-6 font-black text-slate-800 text-sm tracking-tight">{{ $kategori->nama_kategori }}</td>
                                        <td class="px-8 py-6 text-center">
                                            <span class="text-sm font-bold text-slate-600">{{ $kategori->alats_count }} Jenis</span>
                                        </td>
                                        <td class="px-8 py-6 text-center">
                                            <span class="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-widest ring-1 ring-indigo-100">
                                                {{ number_format($kategori->alats_sum_stok ?? 0, 0, ',', '.') }} Unit
                                            </span>
                                        </td>
                                        <td class="px-8 py-6 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <button @click="editData = {id: {{ $kategori->id }}, name: '{{ $kategori->nama_kategori }}'}; editModal = true" class="w-9 h-9 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:bg-indigo-500 hover:text-white transition-all shadow-sm">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                </button>
                                                <form action="{{ route('admin.kategoris.destroy', $kategori) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?')" class="inline">
                                                    @csrf @method('DELETE')
                            ...
                            <form action="{{ route('admin.kategoris.store') }}" method="POST">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" name="nama_kategori" required class="w-full rounded-xl border-slate-200 focus:ring-indigo-500">
                                    </div>
                                </div>
                ...
                <template x-if="editModal">
                    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div class="relative w-full max-w-lg bg-white rounded-lg p-8 shadow-2xl animate-in zoom-in duration-200">
                            <h3 class="text-xl font-bold text-slate-800 mb-6">Edit Kategori</h3>
                            <form :action="'/admin/kategoris/' + editData.id" method="POST">
                                @csrf @method('PUT')
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-bold text-slate-700 mb-2">Nama Kategori</label>
                                        <input type="text" name="nama_kategori" :value="editData.name" required class="w-full rounded-xl border-slate-200 focus:ring-indigo-500">
                                    </div>
                                </div>
                                <div class="flex gap-3 mt-8">
